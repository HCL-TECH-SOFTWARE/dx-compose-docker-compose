...
:: Thise script sets up necessary environment variables to run DX Compose in a docker-compose environment
...
FOR /F "tokens=1,2 delims==" %%A IN (dx.properties) DO (
    ...
    IF "%%A"=="DX_DOCKER_IMAGE_WEBENGINE" SET DX_DOCKER_IMAGE_WEBENGINE=%%B
    IF "%%A"=="DX_DOCKER_IMAGE_DIGITAL_ASSET_MANAGER" SET DX_DOCKER_IMAGE_DIGITAL_ASSET_MANAGER=%%B
    IF "%%A"=="DX_DOCKER_IMAGE_DATABASE_NODE_DIGITAL_ASSET_MANAGER" SET DX_DOCKER_IMAGE_DATABASE_NODE_DIGITAL_ASSET_MANAGER=%%B
    IF "%%A"=="DX_DOCKER_IMAGE_DATABASE_CONNECTION_POOL_DIGITAL_ASSET_MANAGER" SET DX_DOCKER_IMAGE_DATABASE_CONNECTION_POOL_DIGITAL_ASSET_MANAGER=%%B
    ...
    IF "%%A"=="DX_DOCKER_IMAGE_HAPROXY" SET DX_DOCKER_IMAGE_HAPROXY=%%B
    IF "%%A"=="DX_DOCKER_IMAGE_OPENSEARCH" SET DX_DOCKER_IMAGE_OPENSEARCH=%%B
    IF "%%A"=="DX_DOCKER_IMAGE_FILE_PROCESSOR" SET DX_DOCKER_IMAGE_FILE_PROCESSOR=%%B
    IF "%%A"=="DX_DOCKER_IMAGE_SEARCH_MIDDLEWARE" SET DX_DOCKER_IMAGE_SEARCH_MIDDLEWARE=%%B
    IF "%%A"=="DX_DOCKER_IMAGE_PEOPLE_SERVICE" SET DX_DOCKER_IMAGE_PEOPLE_SERVICE=%%B
)
...
SET COMPOSE_FILE=%SCRIPT_DIR%\dx.yaml
 ...
 :: external hostname of DX Compose environment. This normally is just localhost, but may be changed if not running local
SET DX_HOSTNAME=localhost

:: docker image versions
SET DX_DOCKER_IMAGE_CC=%DX_DOCKER_IMAGE_CONTENT_COMPOSER%
SET DX_DOCKER_IMAGE_WEBENGINE=%DX_DOCKER_IMAGE_WEBENGINE%
SET DX_DOCKER_IMAGE_DAM=%DX_DOCKER_IMAGE_DIGITAL_ASSET_MANAGER%
SET DX_DOCKER_IMAGE_DAM_DB_NODE=%DX_DOCKER_IMAGE_DATABASE_NODE_DIGITAL_ASSET_MANAGER%
SET DX_DOCKER_IMAGE_DAM_DB_CONNECTION_POOL=%DX_DOCKER_IMAGE_DATABASE_CONNECTION_POOL_DIGITAL_ASSET_MANAGER%
SET DX_DOCKER_IMAGE_IMAGE_PROCESSOR=%DX_DOCKER_IMAGE_IMAGE_PROCESSOR%
SET DX_DOCKER_IMAGE_RINGAPI=%DX_DOCKER_IMAGE_RING_API%
SET DX_DOCKER_IMAGE_HAPROXY=%DX_DOCKER_IMAGE_HAPROXY%
SET DX_DOCKER_IMAGE_OPENSEARCH=%DX_DOCKER_IMAGE_OPENSEARCH%
SET DX_DOCKER_IMAGE_FILE_PROCESSOR=%DX_DOCKER_IMAGE_FILE_PROCESSOR%
SET DX_DOCKER_IMAGE_SEARCH_MIDDLEWARE=%DX_DOCKER_IMAGE_SEARCH_MIDDLEWARE%
SET DX_DOCKER_IMAGE_PEOPLE_SERVICE=%DX_DOCKER_IMAGE_PEOPLE_SERVICE%
