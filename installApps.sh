...
echo "############################################################################"
echo ""
docker exec dx-webengine sh -c "/opt/openliberty/wlp/usr/svrcfg/bin/product-features/manageSearchV2.sh -Dsearch.input.redirect.version=2 -Dsearch.wcm.version=2 -Dsearch.middleware.ui.uri=$MIDDLEWARE_SERVICE_URI -DENABLE=$ENABLE_SEARCHV2"
echo "############################################################################"
echo ""
docker exec dx-webengine sh -c "/opt/openliberty/wlp/usr/svrcfg/bin/product-features/managePeopleService.sh -DPEOPLESERVICE_ENABLED=$ENABLE_PEOPLE_SERVICE -DPEOPLESERVICE_WEBRESOURCES_URI=$PEOPLE_SERVICE_WEBRESOURCES_URI -DPEOPLESERVICE_UI_CONTEXT=$PEOPLE_SERVICE_CONTEXT_ROOT_UI -DPEOPLESERVICE_API_CONTEXT=$PEOPLE_SERVICE_CONTEXT_ROOT_API -DPEOPLESERVICE_PORTLET_CONTEXT=$PEOPLE_SERVICE_CONTEXT_ROOT_PORTLET"
echo "############################################################################"
